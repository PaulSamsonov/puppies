...
class tsl_puppies_direct_cron_backup
{
    ...
    function delete_drafts(){
        ...
        $this->json_reponse[] = 'Remove First: ' . sizeof($pets);

        foreach( $pets as $pet ) {
            if(! get_post_meta( $pet->ID, '_price', true )) {
                wp_delete_post($pet->ID);
            }
        }
        ...
        $this->json_reponse[] = 'Remove Second: ' . sizeof($pets);
    ...
    public function update_pet_inventory( $all_pet_data )
    {
        ...
        foreach ($all_pet_data as $pet) {
            ...
            if(isset($pet['OrgName'])) update_post_meta( $product_id, 'pdm_pet_breed', $pet['BreedName'] );
